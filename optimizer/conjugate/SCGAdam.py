...
from optimizer.conjugate.scg_param import get_scg1_param_fn,get_scg2_param_fn


class SCGAdam(Optimizer):
    def __init__(self, params, period: int, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.0, amsgrad=False,
                 cg1_type='C1', cg2_type='C1', lam=2.0) -> None:
        if not 0 <= period:
        ...
        super(SCGAdam, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(SCGAdam, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)
            group.setdefault('cg1_type', 'C1')
            group.setdefault('cg2_type', 'C1')
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            errors = []
            scg1_param_fn = get_scg1_param_fn(group['cg1_type'])
            scg2_param_fn = get_scg2_param_fn(group['cg2_type'])
            for p in group['params']:
                ...
                if state['stochastic_cg'] is None:
                    ...
                    state['stochastic_cg'] = -(1 + scg1_param) * grad + scg2_param * scg
                    state['past_grad'] = grad.clone()
